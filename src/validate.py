...
def validate_ticker_present(df: pd.DataFrame) -> pd.DataFrame: # accepts df and returns as a filtered df
...
def get_invalid_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    invalid_high_low = validate_high_low(df) # So all helper func are added here in this func ...
    ...
    invalid_rows = pd.concat([
        ...
        invalid_dates,
        invalid_high_low
        ]).drop_duplicates() # Combine all invalid records and remove rows repeated across validation rules.
    
    return invalid_rows

def get_valid_rows(df: pd.DataFrame) -> pd.DataFrame: 
    invalid_rows = get_invalid_rows(df)
    valid_rows = df.drop(invalid_rows.index) # Removes the invalid rows from the original df / no mutation.
    return valid_rows
